...
app.use('/api', (req, res, next) => {
  if (req.path === '/auth/login' || req.path === '/upload') return next();
...
app.post('/api/deals', async (req, res) => {
  ...
  const infDoc = await db.collection('users').doc(req.body.influencerId).get();
  const brandDoc = await db.collection('users').doc(user.id).get();
  
  const infName = infDoc.exists ? infDoc.data().name : 'Unknown Influencer';
  const brandName = brandDoc.exists ? (brandDoc.data().profile?.company || brandDoc.data().name) : 'Unknown Brand';
...
app.delete('/api/users/influencer/:id', async (req, res) => {
  ...
  if (doc.exists && doc.data().image) {
    try {
      const url = new URL(doc.data().image);
